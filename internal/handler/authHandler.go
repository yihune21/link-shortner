...
func (a *AuthHandler)MiddlewareAuth(w http.ResponseWriter , r *http.Request) {
		...
		user_id,err := jsonwebtoken.ExtractUserIDFromToken(access_token)
		if err != nil{
			WriteError(w , 400 , fmt.Sprintf("Error with extracting user id %v",err))
			return
		}
		user , err := a.as.GetUserById(r.Context() , user_id)
		
        if err != nil{
			WriteError(w , 404 , fmt.Sprintf("Couldn't found user %s" , err))
			return
		}
